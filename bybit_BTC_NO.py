...
# form_class = uic.loadUiType("MA_SELL.ui")[0]
client = bybit.bybit(test=False, api_key="IXSWQSRSWFWTNBGYWW", api_secret="ZZCDVRTJWDKTRDQWGMFZZZOFBORKAXQLIRKF")
    ...
    while True:
        try:
            ...
            for i in range(len(coinName)):
                ...
                wallet = get_MyWallet(coinName[i])
                # print(coinTicker[i], " wallet : ", wallet)

                importance = get_Importance(wallet)

                if coinName[i] == "BTC":
                    if isFirstLoop_BTC:
                        print(coinName[i], " importance : ", importance,
                              " LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG")
                        isFirstLoop_BTC = False
                        if importance >= 25:
                            print(coinTicker[i], "비중이 25%를 넘었다")
                            set_FirstOnlyOrderListInit(coinTicker[i])

                if coinName[i] == "ETH":
                    if isFirstLoop_ETH:
                        print(coinName[i], " importance : ", importance,
                              " LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG LONG")
                        isFirstLoop_ETH = False
                        if importance >= 25:
                            print(coinTicker[i], "비중이 25%를 넘었다")
                ...
                if importance == 0:
                    candle_data = get_ohlcv(coinTicker[i], "1")
                else:
                    candle_data = get_ohlcv(coinTicker[i], "1")
                ...
                def rsi(ohlc: pd.DataFrame, period: int = 14):
                    try:
                        ...
                        print('rsi error :', sys.exc_info()[0])
                ...
                if importance == 0:
                    ...
                    if float(rsi) <= 30:
                        ...
                        set_SpiderLine(wallet, 0, coinTicker[i], leverage[i], True)
                elif importance > 0:
                    # 포지션이 있는 상태
                    # 1.거미줄을 넘기고 더 내려간 경우
                    ...
                    # RSI가 70을 넘긴 경우 전포지션 매도
                    # print(coinTicker[i], " 수익률 : ", self.get_RevenuePercent(currentPrice, coinTicker[i], leverage[i]))
                    # print(coinTicker[i], " 의 importance : ", importance)
                    # print(coinTicker[i], " importance : ", importance)
                    if rsi >= 70:
                        if get_Side(coinTicker[i]) == "SELL":
                            if importance >= 40:
...
def check_SpiderGap(coinTicker):
    aPrice = float(get_AverageUnitPrice(coinTicker))
    activeOrder = get_ActiveOrder(coinTicker)
...
def buy_FirstCoin(walletData, coinTicker, leverage):
    ...
    try:
        ...
        if usedMargin <= 0:
            # 시장가 매수
            client.Order.Order_new(side="Buy", symbol=coinTicker, order_type="Market", qty=orderQty,
                                   time_in_force="GoodTillCancel").result()
            print(coinTicker, " 구매")
    except:
        print('buy_FirstCoin error :', sys.exc_info()[0])

def check_CopyOrderList(coinTicker):
    try:
...
def set_SpiderLine(wallet, currentPrice, coinTicker, leverage, isFirst):
    try:
        global copyOrderList_XRP
        global copyOrderList_ETH
        global copyOrderList_BTC
        ...
        if float(order_margin) <= 0:
            if currentPrice == 0:
                # 평단가를 구함
                entryPrice = get_AverageUnitPrice(coinTicker)
            else:
            ...
            # copyOrderList = []
            check_CopyOrderList(coinTicker)

            for i in range(5):
                if i == 0 and isFirst:
                    price = float(entryPrice) - (float(entryPrice) * (0.005 * (i + 1)))
                elif i == 1 and isFirst:
                    price = float(entryPrice) - (float(entryPrice) * (0.01 * (i + 1))) + (float(entryPrice) * (0.005 * (i + 1)))
                elif isFirst:
                ...
                price = round(price, 4)
                print("price : ", price)
                client.Order.Order_new(side="Buy", symbol=coinTicker, order_type="Limit", qty=orderQty, price=price,
                                       time_in_force="GoodTillCancel").result()
                time.sleep(0.3)
                ...
                if importance >= 25:
                    ...
                    if coinTicker == "XRPUSD":
                        ...
                        print(coinTicker, "set_SpiderLine 티커가 잘못들어왔다!!")
...
def set_FirstOnlyOrderListInit(coinTicker):
    try:
        global copyOrderList_XRP
        global copyOrderList_ETH
        global copyOrderList_BTC
        ...
        for order in activeOrder:
            ...
            if coinTicker == "XRPUSD":
                ...
                print("set_SpiderLine 티커가 잘못들어왔다!!")
...
def check_SpiderLine(cPrice, coinTicker):
    # 포지션이 있는 상태
    # 1.거미줄중간에 올라간 경우
    #   - 매수된 거미줄이 있는지 체크하고 만약 있다면 해당 거미줄의 목표 매도가에 매도 예약
    # 2.거미줄을 넘기고 더 내려간 경우
    ...
    try:
        ...
        if coinTicker == "BTCUSD":
            for list in copyOrderList_BTC:
                ...
                #     client.Order.Order_new(side="Sell", symbol=coinTicker, order_type="Limit", qty=orderQty,
...
def get_CurrentPrice(coinTicker):
    try:
...
def get_TradeQty(walletData, divisionCount, lever, coinTicker):
    try:
        # 511.4538
        # 0.0145를 곱해준다
        # 504
        ...
        currentMarketPrice = get_CurrentPrice(coinTicker)
...
def get_RevenuePercent(currentPrice, coinTicker, leverage):
    try:
        percent = 0
        # (오르거나 떨어진 현재 주식 가격 / 내가 매수한 주식 가격) * 100 - 100
        averageUnitPrice = get_AverageUnitPrice(coinTicker)
        percent = (float(currentPrice) / float(averageUnitPrice)) * 100 - 100
...
def get_AverageUnitPrice(coinTicker):
    try:
...
def cancle_BuyReserve(coinTicker):
...
def get_ActiveOrder(coinTicker):
    try:
...
def get_ohlcv(symbol, interval, end_str=None):
    try:
...
#
#         self.worker = Worker()
#         # self.worker.buyfinished.connect(self.refreshMyCoin)
